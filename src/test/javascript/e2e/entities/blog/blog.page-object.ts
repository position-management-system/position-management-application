...
export class BlogDeleteDialog {
  ...
  private dialogTitle: ElementFinder = element(by.id('positionApp.blog.delete.question'));
...
export default class BlogComponentsPage {
  ...
  async deleteBlog() {
    ...
    const blogDeleteDialog = new BlogDeleteDialog();
    await waitUntilDisplayed(blogDeleteDialog.deleteModal);
    expect(await blogDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/positionApp.blog.delete.question/);
