...
export default class BlogUpdatePage {
  pageTitle: ElementFinder = element(by.id('positionApp.blog.home.createOrEditLabel'));
  ...
  userSelect: ElementFinder = element(by.css('select#blog-user'));
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setHandleInput('handle');
    expect(await this.getHandleInput()).to.match(/handle/);
