...
import com.pms.config.KafkaProperties;
...
class PositionKafkaResourceIT {
    ...
    void setup() {
        ...
        PositionKafkaResource kafkaResource = new PositionKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/position-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/position-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
